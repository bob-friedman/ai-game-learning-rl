'use strict';
// ============================================================
// evaluate.js  —  FIXED: do not modify.
// Runs ai.js (experimental) vs baseline_ai.js (frozen) across
// all scenarios and returns a win_rate score (higher = better).
// Usage: node evaluate.js > run.log 2>&1
// Grep: grep "^win_rate:" run.log
// ============================================================
...
const GAMES_PER_SCENARIO   = 25;   // × 4 scenarios × 2 roles = 200 games total
const MAX_GAME_TURNS       = 120;  // draw after this many turns
const STALEMATE_TURNS      = 30;   // draw if no combat for this many turns
const TIMEOUT_MATERIAL_EDGE = 1.15; // material ratio needed to call timeout as a win
...
function runGame(ai0, ai1, scenarioId) {
    ...
    while (!state.gameOver) {
        if (state.totalTurns >= MAX_GAME_TURNS) {
            // Timeout: award to side with material advantage, else draw
    ...
    return state.gameWinner; // 0, 1, or -1 (draw)
}
...
function evaluateAI() {
    ...
    for (const scenarioId of EVAL_SCENARIOS) {
        ...
        for (let i = 0; i < GAMES_PER_SCENARIO; i++) {
            // New AI as Blue (team 1), baseline as Gold (team 0)
            ...
            // New AI as Gold (team 0), baseline as Blue (team 1)
