'use strict';
// ============================================================
// ai.js  —  EXPERIMENTAL: this is the only file you edit.
// Contains the full AI decision logic for one side.
// The evaluator pits this AI against baseline_ai.js.
// Goal: highest win_rate (0–100). Baseline ≈ 50.
// ============================================================
...
// ---------- Tuneable constants ------------------------------------
// These are the primary knobs. Try changing them first.
...
function calculateAttackValue(state, attacker, target, breachMultiplier = 1.0) {
    ...
    // Absolute priority: defending own HQ
    ...
    // Bonus for targeting capturer near our HQ
    if (defData.capture) {
    ...
    // Counter-attack cost (melee only)
    if (!atkData.ranged) {
    ...
    score += Math.random() * 5; // tiebreak dither
    return score;
}
...
function tryAiAttack(state, unit, conserveDepth = 0, breachMultiplier = 1.0) {
    ...
    // Conserve-mode caution: skip bad trades
...
function runAITurn(state, team) {
    ...
    for (const unit of aiUnits) {
        ...
        // Standing on enemy HQ → begin/continue capture
        ...
        // Try to attack after moving (melee units only)
