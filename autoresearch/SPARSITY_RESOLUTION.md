...
The process begins with the base generator navigating the continuous semantic manifold to propose novel hypotheses. Operating in a continuous vector space, it detects structural similarities between distant domains. Rather than passing these continuous intuitions into testable formal logic via external symbolic engines using Monte Carlo Tree Search, the system utilizes **Reward-Guided Beam Search**. This distinction is significant: the search does not wait for the decoder to commit to one finished sequence and then judge it. Instead, at every generative step the system holds several partial, unfinished candidate sequences open at once. The space evaluated is therefore wider than what the decoder would produce if left to generate a single, uncorrected sequence on its own.

To evaluate qualitative domains—such as rhetorical nuance or logical flow—the engine employs a Qualitative Reward Model (QRM) dynamically during the decoding phase, scoring candidates before decoding is complete rather than after. The QRM maps each partial trajectory into an embedding space populated by historically verified exemplar vectors, so a candidate can be scored, and discarded if necessary, before the decoder has finished producing it. If the geometric distance between a partial trajectory and the ideal exemplar cluster is sufficiently small, that trajectory survives to the next step; if not, it is pruned. This translates subjective human values into a quantifiable geometric topology, allowing the system to natively prune branches for poor tone or weak argumentation without the massive computational friction of a discrete MCTS wrapper.

The final component drives long-term improvement through Reinforcement Learning from Verifiable Feedback (RLVF). Here, the trajectories that survived QRM evaluation are distilled back into the base generator's own weights. This is the mechanism by which the analogical-reasoning structure captured in the QRM's exemplar data (see Section 8) becomes native to the generator itself: the model is not merely filtered by an external reward model at inference time, it is retrained on its own successful, reward-verified outputs, so its semantic manifold is reshaped to favor structurally sound reasoning by default. To prevent mode collapse—where the model endlessly retraces safe, repetitive pathways at the expense of creativity—the architecture applies dynamic Kullback-Leibler divergence penalties. This mathematical anchor forces the model to maximize reasoning rewards without severely deviating from the broad probability distribution of its original pre-training, preserving the stochastic imagination required for future conceptual leaps.
...
This exemplar data serves two distinct purposes. During inference, it defines the geometric standard the QRM uses to score candidate trajectories in Reward-Guided Beam Search (Section 7). Over time, through the RLVF distillation step described in Section 6, it also becomes training data for the generator itself. Curating exemplars that exhibit analogical reasoning is therefore not only a matter of building a scoring reference; it is, by way of that distillation loop, a means of training the decoder to produce analogically structured output natively, rather than relying on external filtering at every step.
...
### Structural Verification as Trajectory Filtering
