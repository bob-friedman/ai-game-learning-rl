...
function resolveCombat(attacker, defender) {
  ...
  const terrainTile = map[defender.y][defender.x];
  const defenseMod = TERRAIN[terrainTile.type].def;
  const attackerHpRatio = attacker.hp / attacker.maxHp;
  const damageDealt = Math.floor(baseDamage * attackerHpRatio * defenseMod);
  const combatLog = {
    type: "combat",
    attacker: attacker,
    defender: defender,
  ...
  if (defender.hp <= 0) {
    ...
    const counterBase = defenderData.damage[attacker.type] || 0;
    const counterDamage = Math.floor(counterBase * (defender.hp / defender.maxHp));
  ...
  ((e.hp -= y),
    log(
      `${TEAMS[t.team]} ${n.name} attacks ${TEAMS[e.team]} ${a.name} for ${y} damage`,
    ));
  const m = Math.abs(t.x - e.x) + Math.abs(t.y - e.y);
  if (e.hp <= 0)
    (log(`${TEAMS[e.team]} ${a.name} destroyed!`),
      (c.deadUnit = e),
      (units = units.filter((t) => t !== e)));
  else if (!a.ranged && 1 === m) {
    const i = Math.floor((a.damage[t.type] || 0) * (e.hp / e.maxHp));
    ((c.counterDamage = i),
      (t.hp -= i),
      log(`${TEAMS[e.team]} ${a.name} counters for ${i} damage`),
      t.hp <= 0 &&
        (log(`${TEAMS[t.team]} ${n.name} destroyed!`),
        (c.deadUnit = t),
        (units = units.filter((e) => e !== t))));
  }
  (actionHistory.push(c), updateUndoButton());
}
function getMovableTiles(unit, allowFriendlyPass = false) {
  ...
  for (visited.add(`${unit.x},${unit.y}`); queue.length > 0; ) {
    ...
    for (const neighbor of neighbors) {
      ...
      const blockingUnit = getUnitAt(neighbor.x, neighbor.y);
      if (blockingUnit && (!allowFriendlyPass || blockingUnit.team !== unit.team))
        continue;
      const totalCost = current.cost + moveCost;
      if (totalCost <= UNITS[unit.type].move) {
        visited.add(key);
        if (
          !blockingUnit ||
          (allowFriendlyPass && blockingUnit.team === unit.team)
        ) {
          if (!blockingUnit || blockingUnit.team !== unit.team) {
            tiles.push({ x: neighbor.x, y: neighbor.y, cost: totalCost });
          }
        }
  ...
  for (a.add(`${t.x},${t.y}`); i.length > 0; ) {
    ...
    for (const s of o) {
      if (s.x < 0 || s.x >= map[0].length || s.y < 0 || s.y >= map.length)
        continue;
      const o = `${s.x},${s.y}`;
      if (a.has(o)) continue;
      const y = map[s.y][s.x],
        c = TERRAIN[y.type].move;
      if (c >= 255) continue;
      const m = getUnitAt(s.x, s.y);
      if (m && (!e || m.team !== t.team)) continue;
      const l = r.cost + c;
      l <= UNITS[t.type].move &&
        (a.add(o),
        (!m || (e && m.team === t.team)) &&
          ((m && m.team === t.team) || n.push({ x: s.x, y: s.y, cost: l })),
        i.push({ x: s.x, y: s.y, cost: l }));
    }
  }
  return n;
}
...
function runAITurn() {
  ...
  const t = units.filter((t) => 1 === t.team && !t.moved),
    e = [],
    n = [],
    a = [];
  t.forEach((t) => {
    const i = UNITS[t.type];
    i.ranged ? e.push(t) : i.capture ? n.push(t) : a.push(t);
  });
  const i = [...e, ...n, ...a];
  if (shouldDefend(1)) {
    log(
  ...
  !(function t() {
    if (0 === i.length || gameOver) return ((aiThinking = !1), void endTurn());
    const e = i.shift();
    if (!units.includes(e)) return void t();
    const n = getStructureAt(e.x, e.y);
    if (n && "hq" === n.type && n.team !== e.team && UNITS[e.type].capture) {
      (queueCapture(e, n), (e.moved = !0), (e.hasMovedThisTurn = !0));
      const a = UNITS[e.type].name,
        i = TEAMS[e.team],
      ...
      return (
        r &&
          ...
          (r.textContent = `${i} ${a} holding position to capture HQ...`)),
        render(),
        void setTimeout(t, 300)
      );
    }
    const a = getAttackTargets(e);
    if (a.length > 0 && !e.hasMovedThisTurn) {
      const n = selectOptimalTarget(e, a);
      if (!n) return;
      const i = getUnitAt(n.x, n.y);
      if (i)
        return (
          resolveCombat(e, i),
          (e.hasAttacked = !0),
          UNITS[e.type].ranged,
          (e.moved = !0),
          (e.hasMovedThisTurn = !0),
          render(),
          void setTimeout(t, 400)
        );
    }
    const r = getMovableTiles(e, !0);
    if (r.length > 0) {
      let n, a, i;
      if (shouldDefend(1)) {
        const t = structures.find((t) => "hq" === t.type && 1 === t.team);
        t
          ? ((n = t.x), (a = t.y), (i = t))
          : ((i = structures.find((t) => "hq" === t.type && null === t.team)),
            i || (i = structures.find((t) => "hq" === t.type && 0 === t.team)),
            i && ((n = i.x), (a = i.y)));
      } else
        ((i = structures.find((t) => "hq" === t.type && null === t.team)),
          i || (i = structures.find((t) => "hq" === t.type && 0 === t.team)),
      ...
      if (void 0 !== n && void 0 !== a) {
        const t = getDistToHQ(n, a),
          i = units.some((t) => 1 === t.team && UNITS[t.type].capture),
          o = (t, e) =>
            structures.some((n) => n.x === t && n.y === e && "hq" === n.type);
        r.sort((n, a) => {
          let r = t[n.y][n.x],
            s = t[a.y][a.x];
          return (
            !UNITS[e.type].capture &&
              i &&
              (o(n.x, n.y) && (r += 500), o(a.x, a.y) && (s += 500)),
            r - s
          );
        });
      }
      const o = r[0];
      (e.x, e.y);
      ((e.x = o.x), (e.y = o.y), (e.moved = !0), (e.hasMovedThisTurn = !0));
      const s = getStructureAt(e.x, e.y);
      if (
        (s && s.team !== e.team && UNITS[e.type].capture && queueCapture(e, s),
        !UNITS[e.type].ranged && !e.hasAttacked)
      ) {
        const n = getAttackTargets(e);
        if (n.length > 0) {
          const a = selectOptimalTarget(e, n);
          if (a) {
