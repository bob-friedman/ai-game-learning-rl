...
function runAITurn() {
  ...
  allAIUnits.forEach(u => {
    ...
    if (uData.ranged) {
      rangedUnits.push(u);
    } else if (uData.capture) {
      capturerUnits.push(u);
    } else {
      otherUnits.push(u);
    }
  });
...
function processNextUnit() {
    ...
    if (standingStruct && 
        standingStruct.type === 'hq' && 
        standingStruct.team !== unit.team && 
    ...
    if (movable.length > 0) {
      ...
      if (structAfterMove &&
          structAfterMove.team !== unit.team &&
          UNITS[unit.type].capture) {
...
function calculateAttackValue(attacker, target, targetPos) {
  ...
  const defMod = TERRAIN[terrain.type].def;
  const damage = Math.floor(baseDamage * hpRatio * defMod);
  const targetCost = UNIT_COSTS[target.type] || 20;
  let gain = damage * (targetCost / 10); 
  if (damage >= target.hp) {
    gain += targetCost * 1.5; 
  }
  if (defData.capture && damage >= target.hp) {
    gain += 50;
  }
  let loss = 0;
  if (!atkData.ranged) {
    ...
    const counterDamage = Math.floor(
      (defData.damage[attacker.type] || 0) * counterHpRatio
    );
  ...
  if (healthRatio < 0.5) {
    const cautionPenalty = (1 - healthRatio) * 40;
    loss += cautionPenalty;
  }
  return gain - loss; 
}
